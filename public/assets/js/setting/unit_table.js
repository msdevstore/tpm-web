$(document).ready(function() {
    $(document).on('click', '.main-table-btn', function (e) {
        if ($(e.target)[0].localName === 'td') {
            let unit_id = $(e.target).parent().attr('data');

            let obj = objArr.filter(item => item.unit_id == unit_id)[0];
            updateValues(obj);
    ...
    $('#main-table-delete').click(function() {
        ...
        if (ID) {
            $.ajax({
                url: `/api/v1/delete_row/unit_tbl/unit_id/${unit_id}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
