$(document).ready(function() {
    $(document).on('click', '.main-table-btn', function (e) {
        if ($(e.target)[0].localName === 'td') {
            let decimal = $(e.target).parent().attr('data');

            let obj = objArr.filter(item => item.decimal == decimal)[0];
            updateValues(obj);
    ...
    function updateValues(obj) {
        $('#decimal').val(obj.decimal);
        $('#fraction').val(obj.fraction);
        $('#fractionN').val(obj.fractionN);
        $('#fractionD').val(obj.fractionD);
        $('#holes').val(obj.holes);
        $('#centers').val(obj.centers);
    }
    ...
    $('#main-table-save').click(function() {
        let obj = {
            ...
            fractionN: $('#fractionN').val(),
            fractionD: $('#fractionD').val(),
            holes: $('#holes').val(),
            centers: $('#centers').val(),
        }
        ...
        if (flag) toastr.info("Please input all data!");
            ...
            $.ajax({
                url: '/api/v1/create/frac_tbl/decimal',
                type: 'post',
                headers: {
    ...
    $('#main-table-delete').click(function() {
        ...
        if (decimal) {
            $.ajax({
                url: `/api/v1/delete_row/frac_tbl/decimal/${decimal}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
